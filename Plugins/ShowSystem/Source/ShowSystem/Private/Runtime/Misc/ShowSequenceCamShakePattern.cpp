...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ShowSequenceCamShakePattern)

#if !IS_MONOLITHIC
UE::MovieScene::FEntityManager*& GEntityManagerForDebugging = UE::MovieScene::GEntityManagerForDebuggingVisualizers;
#endif

UShowSequenceCamShakePattern::UShowSequenceCamShakePattern(const FObjectInitializer& ObjInit)
	: Super(ObjInit)
	, PlayRate(1.f)
	, Scale(1.f)
	, BlendInTime(0.2f)
...
void UShowSequenceCamShakePattern::GetShakePatternInfoImpl(FCameraShakeInfo& OutInfo) const
{
	if (Sequence != nullptr)
	{
...
void UShowSequenceCamShakePattern::StartShakePatternImpl(const FCameraShakePatternStartParams& Params)
{
...
void UShowSequenceCamShakePattern::UpdateShakePatternImpl(const FCameraShakePatternUpdateParams& Params, FCameraShakePatternUpdateResult& OutResult)
{
...
void UShowSequenceCamShakePattern::ScrubShakePatternImpl(const FCameraShakePatternScrubParams& Params, FCameraShakePatternUpdateResult& OutResult)
{
...
bool UShowSequenceCamShakePattern::IsFinishedImpl() const
{
	return (Player == nullptr || Player->GetPlaybackStatus() == EMovieScenePlayerStatus::Stopped);
}

void UShowSequenceCamShakePattern::StopShakePatternImpl(const FCameraShakePatternStopParams& Params)
{
	using namespace UE::MovieScene;
...
void UShowSequenceCamShakePattern::TeardownShakePatternImpl()
{
	using namespace UE::MovieScene;
...
void UShowSequenceCamShakePattern::UpdateCamera(FFrameTime NewPosition, const FMinimalViewInfo& InPOV, FCameraShakePatternUpdateResult& OutResult)
{
	if (!ensure(Sequence))
	{
		return;
...
void UShowSequenceCamShakePattern::UpdatePlayRate(float NewPlayRate)
{
	...
	if (Sequence != nullptr)
	{
		...
		if (UMovieScene* MovieScene = Sequence->GetMovieScene())
		{
			...
			if (bRandomSegment)
			{
				NonConstCameraShakeInfo.Duration = FCameraShakeDuration(RandomSegmentDuration / ActualPlayRate);
			}
			else
			{
				const FFrameRate TickResolution = MovieScene->GetTickResolution();
				const TRange<FFrameNumber> PlaybackRange = MovieScene->GetPlaybackRange();
				const float Duration = TickResolution.AsSeconds(PlaybackRange.Size<FFrameNumber>());
				NonConstCameraShakeInfo.Duration = FCameraShakeDuration(Duration / ActualPlayRate);
			}
		}
	}
}
